...
test("connects PayPal subscriptions, 14-day trials, add-ons, and verified billing state", async () => {
  ...
  assert.match(signup, /\$24\.95\/month/);
  assert.match(signup, /\$149 setup · then \$39\/year renewal/);
  ...
  assert.match(signup, /\$1,164\.28 in first-year included service value/);
  ...
  assert.match(billing, /Over \$1,160 in First-Year Included Service Value · Prices Locked In/);
  ...
  assert.match(billingStore, /removeMyDogPortalDogBreederDocsPacket/);
...
test("serves the public marketing homepage on the platform apex", async () => {
  ...
  assert.match(marketing, /Dog Breeder Web Website Service/);
  assert.match(marketing, /\$24\.95\/month/);
  assert.match(marketing, /Standard \.com renewal is included while Studio remains active after the first year; otherwise it is \$39\/year/);
  assert.match(marketing, /Breeder Website Personalization/);
  assert.match(marketing, /From \$749/);
  assert.match(marketing, /"\$290"/);
  assert.match(marketing, /"\$690"/);
  assert.match(marketing, /"\$1,190"/);
  ...
  assert.match(marketing, /Business Voice setup \+ first-year number — \$176\.88 value/);
  assert.match(marketing, /Managed website service \+ two business emails — \$299\.40\/year value/);
  assert.match(marketing, /\$1,164\.28 first-year service value/);
  assert.match(marketing, /Complete DogBreederDocs editable packet included/);
