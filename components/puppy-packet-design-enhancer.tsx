...
const guides: Array<[RegExp, Guide]> = [
  ...
  [/first 72 hours/i, { label: "FIRST DAYS TOGETHER", summary: "A calm, predictable start helps a young puppy feel secure, eat reliably, rest, and begin bonding.", points: ["Keep the first days quiet and consistent.", "Observe every meal and bathroom break.", "Limit visitors and unnecessary travel."], symbol: "⌂" }],
  ...
  [/pup-lift|hypoglycemia/i, { label: "EMERGENCY READINESS", summary: "Toy-breed puppies can decline quickly when blood sugar drops. Preparation and fast action matter.", points: ["Know the early warning signs.", "Keep the recommended emergency supplies accessible.", "Contact a veterinarian immediately during an emergency."], symbol: "+" }],
  ...
  [/training and puppy behavior/i, { label: "LIFELONG SKILLS", summary: "Kind, consistent training helps a puppy become confident, responsive, and safe in everyday life.", points: ["Keep sessions brief.", "Reward the behavior you want repeated.", "Address guarding or fear early."], symbol: "★" }],
  ...
  [/home safety|poison prevention/i, { label: "PUPPY-PROOFING", summary: "Puppies can reach small spaces and be injured by hazards that may not threaten an adult dog.", points: ["Block falls and escape routes.", "Secure medications and toxins.", "Check recliners and doors before moving them."], symbol: "⚑" }],
  ...
  [/travel and car safety/i, { label: "SAFE TRAVEL", summary: "A secure carrier, planned meals, and clean low-risk stops make travel safer and less stressful.", points: ["Use a secured carrier or rated restraint.", "Keep food, water, records, and emergency supplies together.", "Never leave the puppy in a vehicle."], symbol: "➜" }],
...
function applyDesign(tenant: { name: string; primaryBreed: string; websiteUrl: string; contactEmail: string; contactPhone: string }) {
  ...
  if (cover && cover.dataset.redesigned !== "true") {
    ...
    cover.innerHTML = `<div class="designer-cover-art" aria-hidden="true"><span></span><span></span><span></span></div><div class="designer-cover-topline"><b>${escapeHtml(tenant.name)}</b><em>Personalized Go-Home Collection</em></div><div class="designer-cover-main"><div class="designer-cover-copy"><div class="designer-cover-mark">${escapeHtml(initials(puppy))}</div><p class="designer-cover-kicker">Created especially for ${escapeHtml(family)}</p><h1>${escapeHtml(puppy)}<span>Puppy Care Binder</span></h1><p class="designer-cover-intro">A personal collection of care guidance, emergency preparation, puppy records, family resources, and practical support for the first days home and the years ahead.</p></div><div class="designer-cover-profile"><div><span>PUPPY</span><strong>${escapeHtml(puppy)}</strong></div><div><span>FAMILY</span><strong>${escapeHtml(family)}</strong></div><div><span>DATE OF BIRTH</span><strong>${escapeHtml(birthDate)}</strong></div><div><span>PACKET DATE</span><strong>${escapeHtml(packetDate)}</strong></div></div></div><div class="designer-cover-features"><div><i>01</i><span><b>Everyday Care</b><small>Feeding, routines, training, grooming, and settling in.</small></span></div><div><i>02</i><span><b>Health & Safety</b><small>Emergency readiness, prevention, warning signs, and veterinary care.</small></span></div><div><i>03</i><span><b>Personal Records</b><small>Weights, care history, important documents, and family notes.</small></span></div></div><div class="designer-cover-footer">${[tenant.websiteUrl,tenant.contactEmail,tenant.contactPhone].filter(Boolean).map((item)=>`<span>${escapeHtml(item)}</span>`).join("")}</div>`;
  }
...
export function PuppyPacketDesignEnhancer() {
  ...
  useEffect(() => {
    if (!window.location.pathname.startsWith("/puppy-packet")) return;
    applyDesign(tenant);
    const observer = new MutationObserver(() => applyDesign(tenant));
