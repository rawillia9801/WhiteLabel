...
type Puppy = { id: number; name: string; sex: string; color: string; birthDate: string; birthWeight: number; currentWeight: number; weightUnit: string; status: string; priceCents: number; litterName: string; damName: string; sireName: string };
type Update = { id: number; puppyId: number; title: string; body: string; weekNumber: number | null; weight: number | null; createdAt: string };
type WeightLog = { id: number; puppyId: number; measuredAt: string; weight: number; unit: string; feedingStatus: string; notes: string };
type CareRecord = { id: number; puppyId: number; careType: string; title: string; occurredAt: string; nextDueAt: string; provider: string; medication: string; dosage: string; status: string; notes: string };
...
type PortalData = {
  kennel?: { name: string; primaryColor: string; accentColor: string };
...
type Tab = "overview" | "puppy" | "health" | "updates" | "documents" | "payments" | "schedule" | "transportation" | "messages" | "resources" | "account";
...
const navigation: Array<{ id: Tab; label: string; icon: typeof Home }> = [
  ...
  { id: "health", label: "Health & Growth", icon: HeartPulse },
...
export function FamilyPortalExperience({ token, accountMode = false }: { token: string; accountMode?: boolean }) {
  ...
  const portalBrandName = data.kennel?.name || tenant.name;
  ...
  return <main className="family-portal-shell">
    ...
    <aside id="family-portal-navigation" className={`family-sidebar ${mobileNavOpen ? "open" : ""}`}>
      <div className="family-brand"><span><Dog size={24}/></span><div><b>{portalBrandName}</b><small>PRIVATE PUPPY PORTAL</small></div></div>
    ...
    <section className="family-content">
      <div className="family-mobile-head"><button type="button" aria-controls="family-portal-navigation" aria-expanded={mobileNavOpen} aria-label={mobileNavOpen ? "Close Puppy Portal navigation" : "Open Puppy Portal navigation"} onClick={() => setMobileNavOpen((value) => !value)}>{mobileNavOpen ? <X/> : <Menu/>}</button><b>{portalBrandName} Puppy Portal</b></div>
      ...
      {activeTab === "puppy" && <section className="portal-panel">{data.puppies.length ? data.puppies.map((puppy) => { const weights = (data.weights || []).filter((entry) => entry.puppyId === puppy.id); const care = (data.careRecords || []).filter((entry) => entry.puppyId === puppy.id); const latestWeight = weights[0]; return <div key={puppy.id} style={{marginBottom:18}}><div className="puppy-hero"><span className="puppy-avatar">{puppy.name.slice(0,1).toUpperCase()}</span><div><h2>{puppy.name}</h2><p>{[puppy.sex,puppy.color,puppy.litterName].filter(Boolean).join(" · ")}</p></div><span>{puppy.status}</span></div><div className="facts-grid"><div className="fact"><small>Birthday</small><b>{shortDate(puppy.birthDate)}</b></div><div className="fact"><small>Birth weight</small><b>{puppy.birthWeight ? `${puppy.birthWeight} ${puppy.weightUnit || "lb"}` : "Not recorded"}</b></div><div className="fact"><small>Latest weight</small><b>{latestWeight ? `${latestWeight.weight} ${latestWeight.unit}` : puppy.currentWeight ? `${puppy.currentWeight} ${puppy.weightUnit || "lb"}` : "Not recorded"}</b></div><div className="fact"><small>Dam</small><b>{puppy.damName || "Not recorded"}</b></div><div className="fact"><small>Sire</small><b>{puppy.sireName || "Not recorded"}</b></div><div className="fact"><small>Recorded price</small><b>{money(puppy.priceCents)}</b></div></div>{(weights.length > 0 || care.length > 0) && <div className="content-list" style={{marginTop:13}}>{weights.slice(0,3).map((entry) => <div className="content-row" key={`weight-${entry.id}`}><span className="row-icon"><Scale size={17}/></span><div><b>{entry.weight} {entry.unit}</b><small>{shortDate(entry.measuredAt)}{entry.feedingStatus ? ` · ${entry.feedingStatus}` : ""}</small></div><span className="status-pill good">Weight</span></div>)}{care.slice(0,6).map((entry) => <div className="content-row" key={`care-${entry.id}`}><span className="row-icon"><HeartPulse size={17}/></span><div><b>{entry.title}</b><small>{shortDate(entry.occurredAt)}{entry.medication ? ` · ${entry.medication}${entry.dosage ? ` ${entry.dosage}` : ""}` : ""}</small></div><span className="status-pill good">{entry.status}</span></div>)}</div>}</div>; }) : <div className="empty-state"><Dog size={30}/><h3>No puppy assigned yet</h3><p>Your assigned puppy will appear here when placement is confirmed.</p></div>}</section>}

      {activeTab === "health" && <div style={{display:"grid",gap:16}}><section className="portal-panel"><div className="portal-panel-head"><div><span>HEALTH & VACCINATION</span><h2>Care records</h2><p>Veterinary, vaccination, medication, and care information shared by your breeder.</p></div></div><div className="content-list">{data.careRecords.map((entry) => { const puppy = data.puppies.find((item) => item.id === entry.puppyId); return <div className="content-row" key={`health-${entry.id}`}><span className="row-icon"><HeartPulse size={18}/></span><div><b>{entry.title}</b><small>{[puppy?.name,entry.careType,shortDate(entry.occurredAt),entry.provider,entry.medication ? `${entry.medication}${entry.dosage ? ` ${entry.dosage}` : ""}` : ""].filter(Boolean).join(" · ")}</small></div><span className={`status-pill ${statusClass(entry.status)}`}>{entry.status}</span></div>; })}{!data.careRecords.length && <div className="empty-state"><HeartPulse size={30}/><p>No shared health or vaccination records are available yet.</p></div>}</div></section><section className="portal-panel"><div className="portal-panel-head"><div><span>GROWTH</span><h2>Weight history</h2><p>Growth measurements recorded for your assigned puppy.</p></div></div><div className="content-list">{data.weights.map((entry) => { const puppy = data.puppies.find((item) => item.id === entry.puppyId); return <div className="content-row" key={`growth-${entry.id}`}><span className="row-icon"><Scale size={18}/></span><div><b>{entry.weight} {entry.unit}</b><small>{[puppy?.name,shortDate(entry.measuredAt),entry.feedingStatus,entry.notes].filter(Boolean).join(" · ")}</small></div><span className="status-pill good">Recorded</span></div>; })}{!data.weights.length && <div className="empty-state"><Scale size={30}/><p>No growth measurements are available yet.</p></div>}</div></section></div>}
      ...
      {activeTab === "payments" && <section className="portal-panel"><div className="portal-panel-head"><div><span>PAYMENT RECORD</span><h2>Account summary</h2><p>Payments recorded by your breeder and scheduled account items.</p></div></div><div className="payment-summary"><span><small>Purchase total</small><b>{money(data.payments.saleTotalCents)}</b></span><span><small>Payments recorded</small><b>{money(data.payments.paidCents)}</b></span><span><small>Outstanding</small><b>{money(data.payments.outstandingCents)}</b></span></div><div className="content-list">{data.payments.items.map((payment) => <div className="content-row" key={payment.id}><span className="row-icon"><CircleDollarSign size={18}/></span><div><b>{payment.description}</b><small>{[payment.category,payment.method,shortDate(payment.paidDate || payment.dueDate)].filter(Boolean).join(" · ")}</small></div><div style={{textAlign:"right"}}><b>{money(payment.amountCents)}</b><span className={`status-pill ${statusClass(payment.status)}`} style={{display:"inline-block",marginTop:4}}>{payment.status}</span></div></div>)}{!data.payments.items.length && <div className="empty-state"><WalletCards size={30}/><p>No payments are recorded yet.</p></div>}</div></section>}
      ...
      {activeTab === "messages" && <div style={{display:"grid",gap:16}}><section className="portal-panel"><div className="portal-panel-head"><div><span>CONTACT</span><h2>Reach the team</h2><p>Use the portal form for account-specific questions so the message remains connected to your family record.</p></div></div><div className="contact-grid">{(data.support.email || tenant.contactEmail) && <a className="contact-card" href={`mailto:${data.support.email || tenant.contactEmail}`}><span><Mail size={19}/></span><div><b>Email support</b><small>{data.support.email || tenant.contactEmail}</small></div></a>}{(data.support.phone || tenant.contactPhone) && <a className="contact-card" href={`tel:${data.support.phone || tenant.contactPhone}`}><span><Phone size={19}/></span><div><b>Telephone</b><small>{data.support.phone || tenant.contactPhone}</small></div></a>}</div></section><section className="portal-panel"><div className="portal-panel-head"><div><span>NEW MESSAGE</span><h2>Send a private message</h2><p>Your message will be added to Breeder Portal and the team will receive an email notification.</p></div></div><RequestForm token={token} kind="support" onCreated={load}/></section><section className="portal-panel"><div className="portal-panel-head"><div><span>REQUEST HISTORY</span><h2>Recent requests</h2></div></div><div className="content-list">{data.requests.map((request) => <div className="content-row" key={request.id}><span className="row-icon"><MessageSquareText size={18}/></span><div><b>{request.subject}</b><small>{request.kind === "transportation" ? "Transportation" : "Support"} · {shortDate(request.createdAt)}</small></div><span className={`status-pill ${statusClass(request.status)}`}>{request.status}</span></div>)}{!data.requests.length && <div className="empty-state"><HelpCircle size={28}/><p>No portal requests have been sent yet.</p></div>}</div></section></div>}

      {activeTab === "resources" && <div style={{display:"grid",gap:16}}><section className="portal-panel"><div className="portal-panel-head"><div><span>FAMILY RESOURCES</span><h2>Your connected puppy information</h2><p>Use these shortcuts to reach breeder-published records already connected to your family and puppy.</p></div></div><div className="content-list"><button type="button" className="content-row" style={{width:"100%",border:0,textAlign:"left",cursor:"pointer"}} onClick={() => chooseTab("documents")}><span className="row-icon"><FileText size={18}/></span><div><b>Documents and agreements</b><small>{data.contracts.length + view.additionalDocuments.length} retained file{data.contracts.length + view.additionalDocuments.length === 1 ? "" : "s"}</small></div><ChevronRight size={15}/></button><button type="button" className="content-row" style={{width:"100%",border:0,textAlign:"left",cursor:"pointer"}} onClick={() => chooseTab("health")}><span className="row-icon"><HeartPulse size={18}/></span><div><b>Health and growth records</b><small>{data.careRecords.length} care record{data.careRecords.length === 1 ? "" : "s"} · {data.weights.length} weight entr{data.weights.length === 1 ? "y" : "ies"}</small></div><ChevronRight size={15}/></button><button type="button" className="content-row" style={{width:"100%",border:0,textAlign:"left",cursor:"pointer"}} onClick={() => chooseTab("schedule")}><span className="row-icon"><CalendarDays size={18}/></span><div><b>Appointments and next steps</b><small>{view.upcoming.length} upcoming event{view.upcoming.length === 1 ? "" : "s"}</small></div><ChevronRight size={15}/></button><button type="button" className="content-row" style={{width:"100%",border:0,textAlign:"left",cursor:"pointer"}} onClick={() => chooseTab("messages")}><span className="row-icon"><MessageSquareText size={18}/></span><div><b>Ask your breeder</b><small>Send an account-specific question that remains connected to your family record.</small></div><ChevronRight size={15}/></button></div></section></div>}

      {activeTab === "account" && <div style={{display:"grid",gap:16}}><section className="portal-panel"><div className="portal-panel-head"><div><span>FAMILY ACCOUNT</span><h2>Contact and placement record</h2><p>Contact the team when information needs to be corrected.</p></div></div><div className="family-account-grid"><div className="account-field"><small>Family name</small><b>{data.buyer.name}</b></div><div className="account-field"><small>Email</small><b>{data.buyer.email}</b></div><div className="account-field"><small>Telephone</small><b>{data.buyer.phone || "Not recorded"}</b></div><div className="account-field"><small>Location</small><b>{data.buyer.location || "Not recorded"}</b></div><div className="account-field"><small>Application status</small><b>{data.buyer.applicationStatus || "Not recorded"}</b></div><div className="account-field"><small>Recorded preferences</small><b>{[data.buyer.preferredSex,data.buyer.preferredColor].filter(Boolean).join(" · ") || "Not recorded"}</b></div></div></section><section className="portal-panel"><div className="portal-panel-head"><div><span>ACCOUNT SECURITY</span><h2>Private access</h2><p>Your password is managed by the secure authentication service and is not visible in Breeder Portal.</p></div></div><div className="content-list"><div className="content-row"><span className="row-icon"><ShieldCheck size={18}/></span><div><b>Password-protected account</b><small>Your browser remains signed in for up to 30 days unless you sign out.</small></div><span className="status-pill good">Active</span></div><div className="content-row"><span className="row-icon"><Mail size={18}/></span><div><b>Secure email-link backup</b><small>Use the Puppy Portal sign-in page to request a temporary email link when needed.</small></div><ChevronRight size={15}/></div></div>{accountMode && <form action="/api/portal/auth/logout" method="post" style={{marginTop:14}}><button type="submit" style={{display:"inline-flex",alignItems:"center",gap:7,padding:"11px 14px",border:0,borderRadius:12,background:"#173f46",color:"white",fontWeight:800,cursor:"pointer"}}><LogOut size={16}/>Sign out of this browser</button></form>}</section></div>}
    </section>
  </main>;
}
