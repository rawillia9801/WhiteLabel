...
const views = allViews.filter((view) => {
  if (view.id === "CRM") return publicTenant.features.phoneCenter;
  if (view.id === "Delivery") return publicTenant.features.transportation;
  if (view.id === "Portal") return publicTenant.features.familyPortal;
  if (view.id === "Applications") return publicTenant.features.applications;
  return true;
});
...
const money = (cents: number | null | undefined) => new Intl.NumberFormat(publicTenant.locale, { style: "currency", currency: publicTenant.currency }).format((cents ?? 0) / 100);
...
export default function Home() {
  ...
  return <div className="bos-shell">
    <aside className={`bos-command-bar${sidebarOpen ? " mobile-open" : ""}`}>
      ...
      <div className="bos-sidebar-scroll">
        ...
        <footer className="bos-sidebar-footer">
          <div><a href="/settings/branding"><Palette size={15}/><span>Brand</span></a><a href="/settings/domain"><Globe2 size={15}/><span>Domain</span></a><a href="/support"><Headphones size={15}/><span>Support</span></a></div>
    ...
    <main className="bos-main">
      <a className="bos-profile-pill" href="/account" aria-label="Open My Account, subscriptions, invoices, receipts, and billing"><UserRound size={17}/><span><b>My Account & Billing</b><small>Plan · invoices · receipts · cancel</small></span><ChevronRight size={14}/></a>
      {view !== "Command" && <header className="bos-view-head"><div><small>{groupLabels[activeGroup]} / {activeViewDefinition.label}</small><h1>{viewCopy[view].title}</h1><p>{viewCopy[view].text}</p></div><div>{view === "Applications" ? <><button onClick={() => { window.location.href = "/applications"; }}><ClipboardCheck size={15} /> Application builder + website</button><button className="primary-action" onClick={() => openCreate("buyers", { application_status: "New" })}><Plus size={15} /> New application</button></> : view === "Puppies" ? <><button onClick={() => { window.location.href = "/puppies/website"; }}><Globe2 size={15} /> Puppy Website Builder</button><button className="primary-action" onClick={() => openCreate("puppies")}><Plus size={15} /> Add puppy</button></> : <><button onClick={() => openCreate("transactions", { type: "Payment" })}><ReceiptText size={15} /> Payment</button><button className="primary-action" onClick={() => openCreate(quickResource)}><Plus size={15} /> Add to {activeViewDefinition.label}</button></>}</div></header>}
