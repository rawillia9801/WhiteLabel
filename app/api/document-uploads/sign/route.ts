...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json(await createSignedDocumentUpload({ kind, ownerId: Number(body.ownerId), fileName: String(body.fileName ?? ""), contentType: String(body.contentType ?? ""), sizeBytes: Number(body.sizeBytes), kennelId: session.kennelId }), { status: 201 });
  } catch (error) {
