...
const plans = [
  ...
  ["Studio", "$99", "$990", "RUN YOUR BUSINESS + YOUR BRAND", "Over $1,100 in first-year included services", ["Everything in Professional", "Brand Launch setup — $149 value", "Breeder Website Personalization — $299 value", "Standard .com registration + renewal — $29 value", "Website hosting + two business emails — $215.40/year value", "Business Voice number + custom IVR — $176.88 first-year value", "Phone System Credits — $240/year value", "Connected public puppy data"]],
] as const;

const services = [
  [Globe2, "Website Hosting + Business Email", "$17.95/month · Included with Studio", "Straightforward managed hosting and professional email without a MyDogPortal software subscription. Included with an active Studio subscription.", ["Website hosting", "SSL certificate", "Two branded business email addresses", "Hosting management and basic technical support", "Connection of your existing domain", "Studio value: $215.40/year", "Included with Studio; available standalone for $17.95/month"]],
  [Globe2, "Brand Launch", "$149 one-time · Included with Studio", "Register and configure an available standard .com under your breeder brand. Brand Launch is included with Studio.", ["Available standard .com registration", "Domain and DNS configuration", "SSL connection", "Standard .com registration + renewal: $29 value", "Annual standard .com renewal included while Studio is active; otherwise $29/year", "Brand Launch setup: $149 value with Studio", "Website hosting + two business emails included with Studio", "Premium domains priced separately"]],
  [MonitorSmartphone, "Breeder Website Personalization", "$299 one-time · Included with Studio", "Turn a MyDogPortal website style into your kennel's own polished public presence. Included with Studio.", ["Your kennel identity, colors, photography, and content", "Supported page and layout personalization", "Connected MyDogPortal information", "Studio value: $299", "Included with Studio; available à la carte for Starter and Professional"]],
  ...
  [PhoneCall, "Business Voice", "$69 setup · Included with Studio", "A professionally configured local business voice line with your greeting and breeder information. Voice setup and the local number are included with Studio.", ["Local number: $8.99/month or $99/year outside Studio", "Incoming calls: $0.03/minute", "Outgoing calls: $0.04/minute", "Custom IVR/menu, business hours, voicemail, and routing", "Studio first-year phone system + number value: $176.88", "Phone System Credits: $240/year value with Studio", "Live examples: Willow Creek (276) 250-9512 · Cedar & Creek (276) 276-2757"]],
] as const;
...
export default function MarketingPage() {
  return <main className="marketing-page">
    ...
    <section className="marketing-section marketing-services" id="services"><header><span className="marketing-kicker">À LA CARTE FOR STARTER + PROFESSIONAL</span><h2>Add the business infrastructure your kennel needs.</h2><p>Starter and Professional can add these services individually. Studio includes hosting, two business emails, Brand Launch, supported website personalization, and Business Voice as part of the $99 plan.</p></header><div className="marketing-service-grid">{services.map(([Icon,name,price,text,items])=><article key={name}><header><span><Icon size={21}/></span><div><h3>{name}</h3><strong>{price}</strong></div></header><p>{text}</p><ul>{items.map(item=><li key={item}><Check size={13}/>{item}</li>)}</ul><Link href="/signup">Add to my setup <ArrowRight size={15}/></Link></article>)}</div><aside><ShieldCheck size={18}/><p><b>Over $1,100 in first-year included services.</b> Studio includes $869.28 in brand, website, domain, hosting/email, and phone-system value plus $240/year in Phone System Credits—a $1,109.28 first-year services value before counting the MyDogPortal software itself. Usage above the included amount, premium domains, and ground-up custom websites are billed separately.</p></aside></section>
