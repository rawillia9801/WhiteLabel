...
async function eventExists(title: string, kennelId: string, buyerId?: number | null) {
  const filters = [`select=id`, `title=eq.${encodeURIComponent(title)}`, `limit=1`];
  filters.push(`kennel_id=eq.${encodeURIComponent(kennelId)}`);
...
async function logEmail(title: string, buyerId: number | null | undefined, status: "Completed" | "Failed", notes: string, kennelId?: string) {
...
export async function sendPortalAccountSetupEmail(input: PortalAccountSetupInput) {
  ...
  if (await eventExists(eventTitle, input.kennelId, input.buyerId)) return { sent: false, skipped: "Already sent." };

  const kennelName = input.kennelName.trim() || "Your Breeder";
  const subject = `Create your ${kennelName} Puppy Portal account`;
  const text = [
